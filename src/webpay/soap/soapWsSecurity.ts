// Clase basada en https://github.com/rgcl/webpay-nodejs, que a su vez está basada en https://github.com/FabianBravoA/tbk_node
...
export default class SoapWsSecurity {

    constructor(privateKey, publicCertificate, encoding = 'utf8') {

        this.instantiatePromise = new Promise((resolve, reject) => {
            this.privateKey = ursa.createPrivateKey(privateKey, null, encoding)
            this.publicP12PEM = publicCertificate.toString().replace('-----BEGIN CERTIFICATE-----', '').replace('-----END CERTIFICATE-----', '').replace(/(\r\n|\n|\r)/gm, '')

            this.signer = new SignedXml()
            this.signer.signingKey = this.privateKey.toPrivatePem()
            this.x509Id = "x509-" + generateId()

            pem.readCertificateInfo(publicCertificate, (pemError, pemData) => {
                if (pemError) {
